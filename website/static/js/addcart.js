...
document.querySelector("#chooseCart").addEventListener("click", () => {
  fetch(`/add-cart?q=${itemId}`, {
    method: "POST",
  ...
  fetch("/addcart-data")
    ...
    .then((res) => {
      if (res["authenticated"] === true) {
        if (res["len"] !== "") {
          cartBtn.innerHTML = `
      <input id="addcart_btn" name="addcart_btn"
       type="button" value="Check Out" href="#" 
       class="btn btn-success px-3 w-25 d-flex align-items-center 
       justify-content-center self-align-end cartBtn"/>
      `;
        }
...
const cartHtmlData = (item) => {
  ...
  <div class="row addcartRow" id="${item.id}">
    ...
    <div class="col-2">
      <div class="py-2">
        <img src="../static/img/${item.image_url[1]}" class="addcart_image" alt="">
      </div>
    </div>
